...
protected:
    ...
    /**
     * counts average committee size from received attests or proposals
     * @param blockProposal true if we are looking for block proposal committee size
     * @return average committee size
     */
    int GetAvgCommitteeSize(bool blockProposal=false);
    ...
    * @param message pointer to rapidjson document containing vote
    ...
    unsigned char m_nextVrfSeed[32];    // VRF seed for next Gasper round (X) generated by leaders
